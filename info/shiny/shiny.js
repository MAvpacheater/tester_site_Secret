// Shiny Stats functionality - Creates full page structure
...
const shinyObserver = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
            const shinyPage = document.getElementById('shinyPage');
            if (shinyPage?.classList.contains('active') && !shinyInitialized) {
...
document.addEventListener('DOMContentLoaded', () => {
    const shinyPage = document.getElementById('shinyPage');
    if (shinyPage) {
        shinyObserver.observe(shinyPage, { attributes: true });
    }
});

// Global exports
window.initializeShiny = initializeShiny;
window.generateShinyStats = generateShinyStats;
window.shinyInitialized = shinyInitialized;
